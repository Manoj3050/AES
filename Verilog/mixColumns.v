...
//---------------------------------------------------------------------------------------------------------------------
   ...
   wire       [DATA_WIDTH-1 : 0]      w_mult2 [15:0];
   wire       [DATA_WIDTH-1 : 0]      w_mult3 [15:0];
...
`endif
      ...
      if(~rst_n_i) begin
         ...
         if(process_start_i) begin
            for(i=0;i<4;i=i+1) begin
               r_byte_out[i*4]   <=#1 w_mult2[i*4] ^ w_mult3[i*4+1] ^ w_byte_in[i*4+2] ^ w_byte_in[i*4+3];
               r_byte_out[i*4+1] <=#1 w_byte_in[i*4] ^ w_mult2[i*4+1] ^ w_mult3[i*4+2] ^ w_byte_in[i*4+3];
               r_byte_out[i*4+2] <=#1 w_byte_in[i*4] ^ w_byte_in[i*4+1] ^ w_mult2[i*4+2] ^ w_mult3[i*4+3];
               r_byte_out[i*4+3] <=#1 w_mult3[i*4] ^ w_byte_in[i*4+1] ^ w_byte_in[i*4+2] ^ w_mult2[i*4+3];
            end
   ...
   generate 
      for(mult_2_c=0; mult_2_c <16;mult_2_c = mult_2_c +1 ) begin : MULT_2_LOOP
         mult2
         mult_two_module
         (
         
            .bytes_i       (w_byte_in[mult_2_c]),
            .bytes_o       (w_mult2[mult_2_c])
   ...
   generate 
      for(mult_3_c=0; mult_3_c <16;mult_3_c = mult_3_c +1 ) begin : MULT_3_LOOP
         mult3
         mult_three_module
         (
         
            .bytes_i       (w_byte_in[mult_3_c]),
            .bytes_o       (w_mult3[mult_3_c])
